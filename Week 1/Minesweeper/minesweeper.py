...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        for i in range(cell[0] - 1, cell[0] + 2):
            for j in range(cell[1] - 2, cell[1] + 2):
        ...
        newSentence = Sentence(undeterminedCells, count - count_mines)
        ...
        for i in range(self.height):
            for j in range(self.width):
                if (i, j) not in self.moves_made and (i, j) not in self.mines:
                    possibelMoves.append((i, j))
        if len(possibelMoves) > 0:
            return random.choice(possibelMoves)
        return None
